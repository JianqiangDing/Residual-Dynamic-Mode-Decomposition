# this file is used to test the generate_trajectory_from_points function

import os
import sys

sys.path.append(os.path.join(os.path.dirname(__file__), ".."))

import numpy as np

import ddrv


# test for the Vanderpol system
def test_vanderpol():
    # set the seed for reproducibility
    np.random.seed(42)

    # define the dynamical system
    Vanderpol = ddrv.dynamic.Vanderpol()

    # sample some points from the domain
    points = ddrv.common.sample_box_set(
        domain=[[-1, 1], [-1, 1]],
        num_samples=200,
    )

    print(points.shape, "points.shape")

    # generate the trajectory data
    traj_data_forward = ddrv.common.generate_trajectory_from_points(
        Vanderpol,
        points=points,
        num_steps=100,
        dt=0.05,
        forward=True,
    )
    print(traj_data_forward.shape)
    ddrv.viz.vis_trajectory_2d(traj_data_forward)

    # generate the trajectory data backward
    traj_data_backward = ddrv.common.generate_trajectory_from_points(
        Vanderpol,
        points=points,
        num_steps=100,
        dt=0.05,
        forward=False,
    )
    print(traj_data_backward.shape)
    ddrv.viz.vis_trajectory_2d(traj_data_backward)


# test for the Duffing oscillator system
def test_duffing_oscillator():
    # set the seed for reproducibility
    np.random.seed(42)

    # define the dynamical system
    DuffingOscillator = ddrv.dynamic.DuffingOscillator()

    # sample some points from the domain
    points = ddrv.common.sample_box_set(
        domain=[[-2, 2], [-2, 2]],
        num_samples=100,
    )

    # generate the trajectory data
    traj_data = ddrv.common.generate_trajectory_from_points(
        DuffingOscillator,
        points=points,
        num_steps=100,
        dt=0.05,
        forward=True,
    )
    print(traj_data.shape)
    ddrv.viz.vis_trajectory_2d(traj_data)


# test for the NL_EIG system
def test_nl_eig():
    # set the seed for reproducibility
    np.random.seed(42)

    # define the dynamical system
    NL_EIG = ddrv.dynamic.NL_EIG()

    # sample some points from the domain
    points = ddrv.common.sample_box_set(
        domain=[[-2, 2], [-2, 2]],
        num_samples=100,
    )

    traj_data = ddrv.common.generate_trajectory_from_points(
        NL_EIG,
        points=points,
        num_steps=10,
        dt=0.05,
        forward=True,
    )
    print(traj_data.shape)
    ddrv.viz.vis_trajectory_2d(traj_data)


if __name__ == "__main__":
    test_vanderpol()
    test_duffing_oscillator()
    test_nl_eig()
