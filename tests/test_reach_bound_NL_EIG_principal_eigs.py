# test the reachability verification of the NL_EIG system with the principal eigenpairs
import os
import sys
...
if __name__ == "__main__":
    ...
    time_bounds, status = ddrv.algo.compute_reach_time_bounds(
        ef0_vals, efF_vals, principal_lambdas_ct
    )
    print(time_bounds, status, "time_bounds & status")
    time_bounds_true, status_true = ddrv.algo.compute_reach_time_bounds(
        ef0_vals_true, efF_vals_true, principal_lambdas_ct
    )
    print(time_bounds_true, status_true, "time_bounds_true & status_true")

    T = np.max(time_bounds)
    print(T, "T")
